...
class TongueVision(nn.Module):
    def __init__(self, num_classes=21):
        super().__init__()
        print("Initializing TongueVision Model (Swin + Swin)...")

        # Branch 1: Swin Tiny (Replaced ConvNeXt)
    ...
    def forward(self, x):
        # Swin models output tensors of shape (B, H, W, C)
        f_swin1 = self.branch1(x)['out']
        # Permute to (B, C, H, W) for standard 2D convolution logic in AGFF
        f_swin1 = f_swin1.permute(0, 3, 1, 2)

        f_swin2 = self.branch2(x)['out']
        # Permute to (B, C, H, W)
...
# ==============================================================================
class EarlyStopping:
    def __init__(self, patience=10, min_delta=0, path='TongueVision_Double_Transformer_v2.pth'): # <--- UPDATED FOR ABLATION MATCH (was 7)
...
if __name__ == "__main__":
    ...
    model.load_state_dict(torch.load('transformer_tongue_expert.pth', map_location=DEVICE))
    ...
    early_stopper = EarlyStopping(patience=10, path='TongueVision_Double_Transformer_v2.pth') # <--- UPDATED FOR ABLATION MATCH
    ...
    print("\nTraining Complete. Best model saved as 'TongueVision_Double_Transformer_v2.pth'.")